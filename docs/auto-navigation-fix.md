...
- Added prioritized job detection: running → pending → completed
- If only completed jobs exist, automatically redirects to results page
...
✅ **Auto-Detection:**
1. Visit `/job` without ID → finds ongoing job
2. Visit `/results` without ID → finds completed job
3. Visit `/job` with only completed jobs → redirects to results

✅ **Edge Cases:**
1. No jobs in history → helpful error message
2. Job still running when visiting results → clear message
